...
import controller.tamtru.TamTruDetailController;
...
public class NhanKhauController implements Initializable {
	...
	@FXML
	private void showTamTruDetail(ActionEvent event) {
		NhanKhauModel selected = tvNhanKhau.getSelectionModel().getSelectedItem();
		if (selected == null) {
			new Alert(Alert.AlertType.WARNING, "Vui lòng chọn một nhân khẩu!").showAndWait();
			return;
		}

		try {
			FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/tamtru/TamTruDetail.fxml"));
			Parent root = loader.load();

			TamTruDetailController controller = loader.getController();
			controller.loadData(selected.getId());

			Stage stage = new Stage();
			stage.setTitle("Thông tin tạm trú");
